...
flowchart LR
    subgraph 数学空间
        direction BT
        一阶概念空间 ---|假设|二阶公理空间 ---|逻辑|二阶命题空间
    end
```
- 概念系统：一阶概念空间划分
    - 集合：集合(set)，关系(relation)，函数(function)，等价(equivalence)。
    - 数系：自然数、整数、实数、虚数。一元数、二元数、四元数、八元数
- 公理系统：二阶公理空间划分
- 数学逻辑系统
    - 模型论
    - 证明论
...
### Roadmap
- 基本要点
> 从代数运算到代数结构：群、环、域、模；同态与同构
从有限维空间到无限维空间：为什么要引入无限维空间；度量空间中的收敛性、完备性和紧性；赋范线性空间与Hahn-Banach定理；Hilbert空间与Fourier展开
从函数到算子：函数概念发展的历史简述；从函数到映射与和算子；广义函数(分布)
从序列收敛到网收敛：数列与序列；度量空间中的序列；拓扑空间中的网
从导数到广义导数：从微积分中的导数谈起；广义函数与广义导数；导子；切丛与向量丛
从Newton-Leibniz公式到Stokes公式：Newton-Leibniz公式及其在高维的推广；外微分式和外微分；微分流形上的Stokes公式；Stokes公式的意义
从Taylor公式到学习理论
从矩阵的特征值到算子的谱
从微分方程到动力系统
从随机变量到随机过程
...
# 数学研究
## 数学思想
- 函数和方程思想：描述系统的结构关系和演化规律。
    - 结构关系：相互作用关系、相互约束关系
    - 演化规律：用运动的观点描述系统
- 极限和收敛思想
    - 无限的变化收敛到有限的结果
    - 无限的变化发散为规律的周期、固定的模式、或者不可测的混沌
- 化归思想：化归对象；化归目标；化归路径(命题形式转化、引入辅助元素、代数问题和几何问题互化...)
    - 简单化原则：高次化低次、高维化低维、多元化低元，例如整体法降元降次
    - 熟悉化原则：陌生问题转化为已有成熟解决方案的问题，例如通过对象形式转化突显对象间的本质联系，以便于进一步寻找解决方案。
    - 具体化原则：将抽象问题直接转化或局部转化为关系更明确的具体问题，例如将抽象的语言描述转化为具体的数量和形式、将代数关系转化为几何关系，以明确各个数学对象间的具体关系。
- 特殊化和一般化
    - 从特殊到一般：从简单的特殊情形出发，推广到一般的情形。如数学归纳法、分类讨论法
    - 从一般到特殊：从一般性命题出发直接得到特殊情形的解答，或一般性命题结合特殊条件得到最终解答。


- 类比思想：根据两个不同对象的某些方面（如特征、属性、关系等）的相同或相似，推出它们的其他方面也可能相同或相似的思维方式，它是思维过程中由特殊到特殊的推理。类比可以启迪思维，根据事物间的相似点提出假设和猜想，但不能作为严格的推理方法。
- 渐变和突变、连续和光滑
## AI与数学研究
- [Lean4](/practice/software/lean/lean.md)