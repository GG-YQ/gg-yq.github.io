...
- 启动流程：PC
    |次序|执行程序|物理器件|
    |-|-|-|
    |1|计算机Bootloader(BIOS/UEFI)进行硬件外设初始化、自检、加载引导程序：BIOS加载硬盘开头的MBR记录；UEFI加载ESP分区里的用于加载和引导操作系统的二级bootloader文件，UEFI通过分区表中的GUID来判断ESP分区。|CMOS或者EEPROM存储器|
    |2|引导程序启动操作系统：MBR分区模式下，MBR引导程序从引导分区读取操作系统镜像并启动操作系统；GPT分区模式下，GPT分区的用户分区中包含一个FAT文件系统的ESP分区（EFI System Partition），ESP分区里的二级bootloader负责后续操作系统的引导。|硬盘|
    |*|GRUB||
